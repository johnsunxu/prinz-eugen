...
class Ship:
    def __init__(self,ship,level=120,limit_break=3,affinity=100,oathed=False,retrofit=True,nicknames=False):
        ...
        try:
            ...
            self.stats = Stats(self.id,self.ship)
            self.level = level
    ...
    @property
    def stats(self):
        return self._stats.getStats(self.level,self._limit_break,self.affinity,self.oathed)
...
s = Ship("Jintsuu",level=120,affinity=100,retrofit=True)
print(s.retrofit)
print(s.getRetrofitStats())
print(s.stats)
print(s.id)
print(s._retrofit)
